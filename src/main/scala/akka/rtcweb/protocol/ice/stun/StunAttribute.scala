...
import shapeless._
import scodec._
import scodec.bits.{ByteVector, BitVector, HexStringSyntax}
...
object StunAttributeType {
  ...
  }, codecs.bits(16).xmap[UNKNOWN](code=>UNKNOWN(code),_.code))
  final case class UNKNOWN(code:ByteVector) extends StunAttributeType {
    ...
    def mustUnderstand:Boolean = code < hex"0x8000"
  }
  ...
  case object FINGERPRINT extends StunAttributeType  final case class UNKNOWN(code:ByteVector) extends StunAttributeType {
...
object StunAttribute {
  ...
  implicit val foo: Codec[StunAttribute] = Codec.coproduct[StunAttribute].auto

  //private def subtypeCodec(stunAttributeType:StunAttributeType):Codec[StunAttribute] = Codec.coproduct[StunAttribute].discriminatedBy(provide(stunAttributeType)).auto
   ...
   */
  /*implicit val codec: Codec[StunAttribute] = blockalignBits(
    StunAttributeType.codec >>:~ { t =>
      variableSizeBytes[StunAttribute]({
        "Length" | uint16
      },
      subtypeCodec(t)
      )
 ...
 */
final case class `ERROR-CODE`(errorCode: Int, reasonPhrase: String)
