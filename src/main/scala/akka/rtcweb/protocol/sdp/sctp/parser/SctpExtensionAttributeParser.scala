...
trait SctpExtensionAttributeParser extends {
   ...
   * app [max-message-size] [streams]}}}
   */
  private def `sctpmap-attr`: Rule1[Sctpmap] = rule { atomic("a=sctpmap:") ~ `sctpmap-number` ~ SP ~ app ~ optional(SP ~ integer) ~ optional(SP ~ streams) ~> ((number: Long, app: String, mms: Option[Long], str: Option[Long]) => Sctpmap(number, app, mms)) }
